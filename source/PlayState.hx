...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			...
			{
				defaultCamZoom = 0.9;
				curStage = 'normalchurch';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('sacredmass/churchbest'));
				bg.antialiasing = true;
			...
			{
				defaultCamZoom = 0.9;
				curStage = 'worshipchurch';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('sacredmass/churchworship'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);
			...
			case 'casanova':
			{
				defaultCamZoom = 0.9;
				curStage = 'selchurch';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('sacredmass/base'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);
		...
		switch (curStage)
		{
			...
			case 'spookychurch':
				...
				dad.x = -450;
				dad.y = 100;
				FlxTween.tween(dad, { x: -450, y: 300 }, 5, { type: FlxTween.PINGPONG, ease: FlxEase.sineInOut, loopDelay: 1});
			case 'worshipchurch':
				gf.x -= 350;
				gf.y += 500;
				boyfriend.x -= 200;
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 3 ? "Alt" : storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - KE 1.4.2" : ""), 16);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				case 'spookychurch':
					babyArrow.frames = Paths.getSparrowAtlas('sacredmass/NOTE_assets2');
